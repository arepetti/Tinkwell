...
{
    protected override object? Call(string arg) => Convert.ToBase64String(Encoding.UTF8.GetBytes(arg));
}

sealed class Base64Decode : UnaryFunction<string>
{
    protected override object? Call(string arg) => Encoding.UTF8.GetString(Convert.FromBase64String(arg));
}
