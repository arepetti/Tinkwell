        ...
        PluginResolver? pluginResolver = null)
    {
        ...
        foreach (var assemblyName in assemblyNames.Distinct(StringComparer.OrdinalIgnoreCase))
        {
            try
            {
                ...
                if (asm is null)
                {
                    // Use AssemblyName.Name — the string ctor misparses dotted simple names (e.g. "A.B" → "A").
                    var simpleName = Path.GetFileNameWithoutExtension(dllName);
                    asm = Assembly.Load(new AssemblyName { Name = simpleName });
                }
